    ...
    <nav className="h-screen w-16 bg-transparent flex flex-col items-center py-4 gap-4 border-r-2 border-[var(--color-border)]">
      ...
      <div className="relative group">
        <button
          ...
          className={`w-12 h-12 flex items-center justify-center rounded-full hover:bg-[var(--color-muted-surface)] overflow-hidden bg-[var(--color-muted-surface)] ${isProfileOpen ? 'animate-avatar-pulse' : ''} interactive-glow-effect-no-scale`}
          aria-label="Ver perfil e histórico"
          aria-describedby={profileTooltipId}
        >
          {avatar ? (
            <img src={avatar} alt="Avatar do usuário" className="w-full h-full object-cover animate-nav-pulse" />
          ) : (
            <AstronautIcon className="w-8 h-8 animate-nav-pulse" />
          )}
        </button>
        <div
          id={profileTooltipId}
      ...
      <div className="relative group">
        <button
          ...
          className="p-3 rounded-full hover:bg-[var(--color-muted-surface)] interactive-glow-effect-no-scale"
          aria-label="Configurações"
          aria-describedby={settingsTooltipId}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-[var(--color-accent)] animate-nav-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
